...
void *limitmalloc_open(limitmalloc_pool *p,int64 len)
{
  ...
  if (len < 0) { errno = error_nomem; return 0; }
  if (len == 0) len = 1; /* protect bad allocators */
  if (len > 1073741823) { errno = error_nomem; return 0; } /* protect bad allocators */

  for (q = p;q;q = q->via)
    if (len > q->limit - q->allocated) { errno = error_nomem; return 0; }
